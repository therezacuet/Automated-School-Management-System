...
<html>
	...
	<body>
		...
		<div class="admin_area">
			<h2 style="position: absolute; font-size: 22px; font-weight: bold;">Book Borrow</h2>
			<table style="width: 80%; margin: 100px auto;">
				...
				<tr>
					...
					<td style="width:20%;font-size:20px;font-weight:bold">Date</td>
				...
				<tr>
					...
					<td><input  type="text" name="borrow_date" class="ins_input_sm" id="datepicker" style="width: 94%; margin: 0px;"  value="<?php echo date('m').'/'.date('d').'/'.date('Y');?>" required></td>
...
<?php
	if (isset($_POST['submit'])) {
		...
		if ($quentity < 1) {
			echo "<script>alert('This Book is Out of Stock','_self')</script>";
		}else{
			...
			$sql="INSERT INTO `borrow` (`id`, `book_id`, `borrower_type`, `borrower_id`, `date_borrow`) VALUES (NULL, '$book_id', '$borrower_type', '$borrower_id', '$borrow_date')";
			$sql_run = mysqli_query($con, $sql);
			if ($sql_run) {
