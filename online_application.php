		...
		<div class="admin_area">
			...
			<table style = "width: 850px;margin: 35px auto;color: rgb(0, 0, 0);font-size: 18px;text-align: center;">
				<tr style ="height: 35px;background-color: rgb(235, 235, 235);">
					...
					<?php
					$sql = "SELECT * FROM `application` WHERE app_type = 'other' ORDER BY `application`.`app_id` DESC";
					...
					while ($sql_get = mysqli_fetch_array($sql_run)) {
						...
						<tr>
							...
							<?php if ($app_stats == 10) {
								?>
									<td style ="border: 1px solid rgb(170, 164, 170);"><a href="view_application.php?app_id=<?php echo $app_id?>&app_type=O&stu_id=<?php echo $student_id;?>" style="background-color: rgb(57, 114, 181);border-radius: 3px;padding: 4px 13px;text-align: center;color: rgb(255, 255, 255);" hover="background-color:#0E5E9F;">View</a></td>

								<?php
							}else if($app_stats == 0){
								?>
									<td style ="border: 1px solid rgb(170, 164, 170);"><h4 style="background-color: red; color: white; width: 88px; padding: 5px 14px; margin: 0px auto; border-radius: 4px; text-align: center;"><a href="view_application.php?app_id=<?php echo $app_id?>&app_type=O&stu_id=<?php echo $student_id;?>" style="color: rgb(255, 255, 255);">Rejected</a></h4> </td>
								<?php
							}else if ($app_stats == 1) {
								?>
									<td style ="border: 1px solid rgb(170, 164, 170);"><h4 style="background-color: green; width: 88px; color: white; margin: 0px auto; padding: 5px 10px; border-radius: 4px;"><a href="view_application.php?app_id=<?php echo $app_id?>&app_type=O&stu_id=<?php echo $student_id;?>" style="color: rgb(255, 255, 255);">Accepted</a></h4> </td>
